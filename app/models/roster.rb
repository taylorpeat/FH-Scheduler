class Roster < ActiveRecord::Base
  ...
  def mem_position_ids(player_id)
    @mem_position_ids ||= Hash.new { |hash, player_id| hash[player_id] =
                                                       mem_players.find(player_id).position_ids }
    @mem_position_ids[player_id]
  end

  def mem_team_game(team, mem_day)
    team_day = [team, mem_day]
    @mem_team_game ||= Hash.new { |hash, team_day| hash[team_day] =
                                                   team_day[0].games.find_by(date: team_day[1]) }
